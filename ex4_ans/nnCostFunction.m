...
% regularison
Theta1p = Theta1;
Theta1p(:, 1) = zeros(size(Theta1, 1), 1);
Theta2p = Theta2;
Theta2p(:, 1) = zeros(size(Theta2, 1), 1);

J = J + lambda / (2*m) * (sum(sum(Theta1p .* Theta1p)) + sum(sum(Theta2p .* Theta2p)) );
...
Theta1_grad = Theta1_grad + lambda/m * Theta1p;
Theta2_grad = Theta2_grad + lambda/m * Theta2p;
